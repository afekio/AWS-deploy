                        ...
                        if all([user_id, file_name, file_type, s3_key]):
                            ...
                            with app.app_context():
                                try:
                                    ...
                                    # --- FIXED: Delete the message ONLY after successful DB commit ---
                                    sqs_client.delete_message(
                                        QueueUrl=SQS_QUEUE_URL,
                                        ReceiptHandle=receipt_handle
                                    )
                                    auth_logger.info(f"[SQS TASK] Message {message.get('MessageId')} successfully deleted from queue.")

                                except Exception as e:
                                    db.session.rollback()
                                    auth_logger.error(f"[SQS TASK] DB Error saving metadata: {e}")
                                    # Notice we DO NOT delete the message here. It will return to the queue for retry.
                        else:
                            auth_logger.warning(f"[SQS TASK] Missing required fields in message: {task_data}")
                            # If the payload is completely invalid, we should delete it so it doesn't block the queue forever
                            sqs_client.delete_message(
                                QueueUrl=SQS_QUEUE_URL,
                                ReceiptHandle=receipt_handle
                            )
...
def get_profile(current_user):
    try:
        ...
        for d in deployments:
            try:
                # The S3 check correctly uses the S3 key, ensuring we look in the right place
                s3_client.head_object(Bucket=S3_BUCKET_NAME, Key=d.s3_key)
                valid_deployments.append(d)
            except ClientError as e:
                error_code = e.response['Error']['Code']
                if error_code == '404':
                    auth_logger.info(f"Auto-Sync: REAL 404 - File '{d.s3_key}' truly missing from S3. Removing from DB.")
                    db.session.delete(d)
                    db_changed = True
                elif error_code == '403':
                    auth_logger.error(f"Auto-Sync: REAL 403 - Permission Denied for '{d.s3_key}'. I am NOT deleting the DB record!")

                else:
                    auth_logger.warning(f"S3 Head Error for {d.s3_key}: {e}")
                    valid_deployments.append(d)

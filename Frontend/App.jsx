...
const App = () => {
  ...
  const [authStatus, setAuthStatus] = useState('offline');
  ...
  // --- Service Health Check (Nginx Routed) ---
  useEffect(() => {
    const checkServices = async () => {
      try {
        const backendRes = await fetch('/api/provision/health');
      ...
      try {
        ...
        setAuthStatus(authRes.ok ? 'online' : 'offline');
      } catch (error) {
    ...
    const interval = setInterval(checkServices, 10 * 60 * 1000);
  ...
  const handleAuthSubmit = async (e) => {
    ...
    // Consistent routing for Auth Service
  ...
  const loadProfile = async () => {
    setSubmittedJson(null);
    try {
      const res = await fetchWithAuth('/api/auth/user/profile');
  ...
  const handlePwdChangeSubmit = async (e) => {
    ...
    try {
      const res = await fetchWithAuth('/api/auth/user/change-password', {
        method: 'POST',
      ...
      if (res.ok) {
        setPwdMessage({ text: 'Password reset successfully, logging out...', type: 'success' });
  ...
  const fetchFileContent = async (fileId, action = 'view') => {
    // Corrected path to include /auth prefix for Nginx routing
    const res = await fetchWithAuth(`/api/auth/user/file/${fileId}?action=${action}`);
  ...
  const styles = `
    ...
    .status-container { position: fixed; top: 20px; left: 20px; background-color: #ffffff; border: 1px solid #e0e0e0; border-radius: 8px; padding: 12px 16px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.05); z-index: 1000; display: flex; flex-direction: column; gap: 8px; }
    .status-row { display: flex; align-items: center; font-size: 14px; }
    ...
    .status-dot.offline { background-color: #ef4444; box-shadow: 0 0 8px rgba(239, 68, 68, 0.5); }
    .status-label { font-weight: 600; color: #374151; margin-right: auto; min-width: 70px; }
    .status-text { font-weight: 500; text-transform: capitalize; }
    .status-text.online { color: #22c55e; }
    .status-text.offline { color: #ef4444; }
    .site-footer { position: fixed; bottom: 0; left: 0; width: 100%; text-align: center; padding: 15px 0; font-size: 14px; color: #6b7280; background-color: rgba(255, 255, 255, 0.8); backdrop-filter: blur(4px); border-top: 1px solid #e5e7eb; z-index: 1000; }
    ...
    <div className="page-wrapper">
      ...
      <div className="status-container">
        <div className="status-row">
          ...
          <span className={`status-text ${backendStatus}`}>{backendStatus}</span>
        </div>
        <div className="status-row">
          <span className={`status-dot ${authStatus}`}></span>
          <span className="status-label">Auth</span>
          <span className={`status-text ${authStatus}`}>{authStatus}</span>
        </div>
      </div>
      ...
      <footer className="site-footer">
        &copy; {new Date().getFullYear()} Afek Gamliel. All rights reserved.
      </footer>
    </div>
  );
};
